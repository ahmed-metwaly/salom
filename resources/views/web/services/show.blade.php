            ...
            <div class="container">
                <div class="row py-md-4 py-0">
                    <div class="col-md-6 col-sm-12 d-flex align-items-center wow slideInRight">
                        <h2 class="h3 color-c5 font-weight-bold "> {{ $service->name }} </h2>
                    </div>
                    <div class="col-md-6 col-sm-12 mt-0 mt-3 text-md-left text-right wow slideInLeft">
                        ...
                        <a class="h5 color-c5 font-weight-bold" href="{{ route('web.companies.show', Hashids::encode($service->company->id)) }}">{{ $service->company->name }} / </a>
                        <span class="h5 color-c5 font-weight-bold">{{ $service->name }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="center">
        <div class="container">
            <div class="text-center my-5 wow fadeInDown" data-wow-duration="2s">
                <h2 class="color-c5">
                    {{ $service->name }}/
                    <a class="color-c5" href="{{ route('web.companies.show', Hashids::encode($service->company->id)) }}">{{ $service->company->name }}</a>
                </h2>
                <img class="img-fluid" src="{{ URL::asset('public/web/img/line.png') }}" alt="">
            </div>
            <div class="row my-5">
                <div class="col-12 mb-5 img-center wow zoomIn" data-wow-duration="2s" style='background: url("{{ url('public/uploads/services/' . $service->photo) }}") no-repeat center center'></div>
                ...
                <div class="col-md-6 col-sm-12 mb-md-0 mb-5">
                    <div class="bg-f6 p-4 font-24 wow fadeInRight" data-wow-duration="2s">
                        <div class="serv-price bg-danger p-2">
                            <span class="text-left text-white h5">{{ $service->price }} ر.س </span>
                        </div>
                        <h4 class="d-inline-block color-c5 font-weight-bold mb-4 mt-5">{{ $service->name }}</h4>

                        <span class="text-left checked float-left font-weight-bold mb-5 mt-5 font-18">
                            {{ $service->ratings()->count() ? $service->ratings()->avg('stars_count') : '0' }}
                        ...
                        <div>
                            ...
                            <p class="color-11 mb-4 font-18 d-inline-block mr-3">
                                {{ explodeByColon( minutesToHours( $service->interval))[1] }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <span class="btn {{ $isOpen > 0 ? 'bg-success' : 'bg-danger' }} text-white ml-3 mr-3 d-inline-block rounded-0">
                                <i class="fa {{ $isOpen > 0 ? 'fa-check' : 'fa-times' }} ml-1"></i>
                                {{ $isOpen > 0 ? 'نعم' : 'لا' }}
                            </span> --}}
                        </div>

                        <div>
                            <h3 class="color-c5 font-weight-bold mb-4 font-18 d-inline-block">مواعيد عمل المشغل :</h3><br>
                            @if(count($workDay) > 0)
                                @foreach($workDay as $work)
                                    <p class="color-11 mb-2 font-18 d-inline-block mr-3">
                                        <div class="row">
                                            <div class="col-md-10">
                                                <label class="form-check-label font-18">
                                                    {{-- <a class="work_intervals">
                                                        {{-- <input full_time="{{ $work->time_from }}" date="{{ getNextDate($work) }}" day="{{ $work->day->en_day }}" type="radio" name="work_times" class="form-check-input" style="margin-top: 10px" {{ isValidReservation($work, $service->company->id) ? '' : 'disabled'}}> --}}
                                                    {{-- </a>  --}}
                                                    -
                                                     {{ $work->day->day }} من
                                            ...
                                            {{-- <div class="col-md-1">
                                                <span class="btn {{ isValidReservation($work, $service->company->id) ? 'bg-success' : 'bg-danger'}} btn-sm text-white ml-3 mr-3 d-inline-block rounded-0" style="font-size: 12px;">
                                                    {{ isValidReservation($work, $service->company->id) ? 'متاح' : 'محجوز'}}
                                                </span>
                                            </div> --}}
                                        </div>
                                    </p>
                ...
                <div class="col-md-6 col-sm-12">
                    <div class="bg-f6 p-4 wow fadeInLeft" data-wow-duration="2s">
                        ...
                        <a class="btn bg-c5 text-white btn-hover px-3 font-weight-bold rounded-0" href="{{ route('web.orders.create', Hashids::encode($service->id)) }}">إحجز الأن</a>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            $('.work_intervals').on('click', function() {
                ...
                if( work_input.is(':checked') ) {
                    ...
                    $.ajax({
                        url: "{{ url('/') }}" + "/orders/store-session-date" ,
                        type: "POST",
