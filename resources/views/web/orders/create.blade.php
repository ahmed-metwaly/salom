...
{{--    <link rel="stylesheet" href="{{ URL::asset('public/web/css/bootstrap-datetimepicker.min.css') }}">--}}
    <link rel="stylesheet" href="{{ URL::asset('public/web/css/bootstrap-datepicker.css') }}">
    <link rel="stylesheet" href="{{ URL::asset('public/web/css/bootstrap-timepicker.css') }}">
@endsection
...
@section('content')
                ...
                <div class="col-12 mb-5 pb-5">
                    <div class="bg-f6 p-4 wow fadeInLeft">
                        <div class="serv-price bg-danger p-2">
                            <span id="service-price" class="text-left text-white h5">{{ $service->price }} ر.س </span>
                        </div>
                        <div class="row mt-5">
                            <div class="col-md-5 mb-4">
                                <img class="img-fluid w-100" style="height: 315px" src="{{ url('public/uploads/services/' . $service->photo) }}" alt="">
                            </div>
                            <div class="col-md-7">
                                <div class="form-group col-md-6 col-sm-12 col-xs-12 wow zoomIn">
                                    ...
                                    <div class="datepicker input-group date form-group mb-3 border rounded-0 p-0">
                                        <input id="date" value="{{ $defaultDate }}" type="text" class="form-control border-0" required>
                                ...
                                <div class="form-group col-md-6 col-sm-12 wow zoomIn">
                                    ...
                                    <div class="bootstrap-timepicker timepicker input-group form-group box mb-3 border rounded-0">
                                        <input id="timepicker" value="{{ $defaultTime }}" type="text" class="form-control border-0">
                                        <div class="input-group-addon bg-white border-0">
                                ...
                                <div class="col-12 mt-3 wow zoomIn">
                                    <button id="confirm-order" companyName="{{$service->company->name}}" class="btn bg-c5 font-18 py-2 px-5 text-white btn-hover">تأكيد حجز الخدمة</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @include('web.orders.map_confirm_modal')
    @include('web.orders.confirm_modal')
    @include('web.orders.invalid_date_modal')
...
<script src="{{ URL::asset('public/web/js/bootstrap-datepicker.js') }}"></script>
<script src="{{ URL::asset('public/web/js/bootstrap-timepicker.js') }}"></script>
    ...
    <script>
        function calcPrice(individual_num) {
            ...
            var price = parseFloat(individual_num) * parseFloat("{{ $service->price }}");
            return price;
        }
        ...
        $(document).ready(function () {
            ...
            //check-if-date-valid......
                ...
                var companyId = "{{ $service->company->id }}";

                $.ajax({
                    url: "{{ url('/') }}" + "/orders/validate-date" ,
                    type: "POST",
                ...
                .done(function(reseived_data) {
                    ...
                    if(parsed_data.code === '4'){
                        ...
                        localStorage.setItem('orderPrice', "{{ $service->price }}");
