...
?>
    @if (session()->has('new_user') || session()->has('new_social_user') || session()->has('user_guest') || session()->has('alert') || session()->has('not_conf') || session()->has('not_status'))
        ...
        <div class="container  text-center" style=" max-height: 40px;">
            <div class="dropdown d-md-inline-block mb-3 ml-md-4 ml-0 wow zoomIn" data-wow-delay=".4s" style="/* visibility: visible; */animation-delay: 1.7s;animation-name: zoomIn;text-align: center !important;">
                <p style="color:  #c5a46d;text-align: center !important;">
                    ...
                    @elseif(session()->has('new_social_user'))
                        {{ session('new_social_user') }}

                    @elseif(session()->has('user_guest'))
                        {{ session('user_guest') }}

                    @elseif(session()->has('alert'))
                        {{ session('alert') }}

                    @elseif(session()->has('not_conf'))
                        {{ session('not_conf') }}

                    @elseif(session()->has('not_status'))
                        {{ session('not_status') }}
...
<div class="top bg-e2 pt-3">
    ...
    z-index: 9999;">
            @if(Auth::check())
                <img class="user-img rounded-circle" src="{{ starts_with(Auth::user()->photo, 'http') ? Auth::user()->photo : url("/public/uploads/users/". Auth::user()->photo) }}" alt="">
                ...
                <div class="dropdown-menu p-3 bg-e2" aria-labelledby="dropdownMenuButton">
                    <ul class="list-unstyled pr-0">
                        @if(Auth::user()->user_type == '3')
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('create-profile') }}">حسابى</a>
                            </li>
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('web.orders.index') }}">حجوزاتي</a>
                            </li>
                        @elseif(Auth::user()->user_type == '2')
                            ...
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('companyDashboard') }}">لوحة التحكم</a>
                            </li>
                        @elseif(Auth::user()->user_type == '1')
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('admin-edit', Auth::user()->id) }}">حسابى</a>
                            </li>
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('web.orders.index') }}">حجوزاتي</a>
                            </li>
                            <li class="my-2">
                                <a class="text-dark" href="{{ route('dashboard') }}">لوحة التحكم</a>
                            </li>
                        @endif
                        <li class="my-2">
                            <a class="text-dark" href="{{ route('change-password-form') }}">تغيير كلمة المرور</a>
                        </li>
                        <li class="my-2">
                            <a class="text-dark" href="{{ route('logout') }}">تسجيل الخروج</a>
                        </li>
                    </ul>
                </div>
            @else
...
{{--                    {!! Form::open([ 'url' => route('login')]) !!}--}}
                        ...
                        {{--<div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">--}}
                            {{--<label for="email" class="font-14">الإميل أو رقم الجوال</label>--}}
                            {{--<input name="email" value="{{ old('email') }}" type="text" class="form-control font-14" id="email" aria-describedby="emailHelp" placeholder="" required="required">--}}
                        ...
                        {{--</div>--}}
                        {{--<div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">--}}
                            {{--<label for="password" class="font-14">الباسورد</label>--}}
                            {{--<input name="password" type="password" class="form-control font-14" id="password" placeholder="" required="required">--}}
                            ...
                            {{--@endif--}}
                        {{--</div>--}}
                        <a href="{{ route('get-login') }}" class="btn bg-c5 text-dark font-14 rounded-0">تسجيل دخول</a>
...
{{--                    {!! Form::close() !!}--}}
        ...
        <a style="direction: ltr" href="del:{{ settings()['phone'] }}" class="text-dark d-md-inline-block d-block mb-3 ml-md-4 ml-0 wow zoomIn" data-wow-delay=".5s">
            {{ settings()['phone'] }}
            <i class="fa fa-phone fa-lg fa-flip-horizontal color-c5 ml-2"></i>
        </a>
        <a href="mailto:{{ settings()['email'] }}" class="text-dark d-md-inline-block d-block mb-3 wow zoomIn" data-wow-delay=".6s">
            <i class="fa fa-envelope fa-lg color-c5 ml-2"></i>
            {{ settings()['email'] }}
        </a>

        <ul class="{{ \Auth::user() ? 'pt-3' : '' }} social list-unstyled mb-0 pr-0 d-md-inline-block float-md-left wow zoomIn" data-wow-delay=".6s">
...
<div class="bottom bg-1c py-3" id="bottom">
    <nav class="navbar navbar-expand-lg">
        <div class="container">
            <!-- Logo -->
            <a href="{{ route('home') }}"  class="navbar-brand font-48 text-white mr-0 wow zoomIn" data-wow-delay=".4s">
                {{--<img class="ml-sm-4 ml-2" src="{{ URL::asset('public/web/img/logo.png') }}" alt="">--}}
                {{ settings()['name'] }}
            </a>
